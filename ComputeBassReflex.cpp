...
ComputeBassReflex::ComputeBassReflex()
{
    printf("ConstructorOnBassReflex\n");
    this->countLinks = 0;
};
void ComputeBassReflex::setMeasures(int height, int width, int length, int measureSubwoofer, int lengthSub)
{
    this->height = height;
    this->width = width;
    this->length = length;
...
void ComputeBassReflex::printMeasuresBox()
{
    cout << "Volume\t" << volume << '\n';
...
int ComputeBassReflex::getVBox(int measureSub)
{
    switch (measureSub)
    {
    case 8:
...
void ComputeBassReflex::computeDirtyMeasuresBox()
{
...
HRESULT_ ComputeBassReflex::QueryInterface_(IID_ &iid, void **ppv)
{
    printf("QueryInterfaceBassReflex:%d\n", iid);
    if (iid == 0)
    {
...
ULONG_ ComputeBassReflex::AddRef_()
{
    this->countLinks++;
    printf("AddRefBassReflex -> %d\n", this->countLinks);
    return 0;
}

ULONG_ ComputeBassReflex::Release_()
{
    this->countLinks--;
    printf("ReleaseBassReflex -> %d\n", this->countLinks);
